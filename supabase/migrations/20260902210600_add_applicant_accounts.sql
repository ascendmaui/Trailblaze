alter table public.career_applications
  ...
  add column if not exists applicant_account_created_at timestamptz;

create index if not exists applications_applicant_user_idx on public.career_applications (applicant_user_id);
...
drop policy if exists applications_applicant_read on public.career_applications;
create policy applications_applicant_read on public.career_applications
for select to authenticated
using (applicant_user_id = (select auth.uid()));

drop policy if exists assessments_applicant_read on public.interview_assessments;
create policy assessments_applicant_read on public.interview_assessments
for select to authenticated
using (
  exists (
    select 1
    from public.career_applications
    where career_applications.id = interview_assessments.application_id
      and career_applications.applicant_user_id = (select auth.uid())
  )
);
